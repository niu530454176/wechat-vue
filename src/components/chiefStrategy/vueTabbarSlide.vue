  ...
  <div class="tabbar-slide-wrapper">
    <div class="swiper-container" :class="options.container">
      <div class="swiper-wrapper">
        <div :style="[slideStyle, {'color': (index == slideOptions.slideIndex) ? slideStyle.checkedColor : slideStyle.color}]" :class="[index == slideOptions.slideIndex ? 'swiper-slide-checked' : '', 'swiper-slide']" :data-id="options.slideId[index]" v-for="(item, index) in options.slideData" :key="index">{{item}}</div>
        <!-- 下划线 -->
    ...
    data () {
      return {
        ...
        slideStyle: {
          ...
          //字体大小
          fontSize: this.options.fontSize || '.44rem',
          //字体格式
          fontFamily: this.options.fontFamily || 'Microsoft YaHei',
          //默认字体颜色
          color: this.options.color || '#333',
          //选中字体颜色
          checkedColor: this.options.checkedColor || '#fa424e'
        },
        downLineStyle: {
          //下划线高度
          downLineHeight: this.options.downLineHeight || '2px',
          //下划线颜色
          downLineColor: this.options.downLineColor || '#fa424e',
        },
        //选项
        slideOptions: {
